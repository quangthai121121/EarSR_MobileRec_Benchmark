"""Benchmark adapter: folder-based SPAN x{2,3,4} inference.

Avoids `import basicsr` package init (SPAN fork lacks vgg_arch / full BasicSR).

CLI (configs/benchmark*.yaml):
  python external/SPAN/main_test.py --scale 4 --model_path ... --input ... --output ...
"""
...
def load_span_class():
    """Load SPAN arch without executing basicsr/__init__.py."""
    ...
    span_mod = _load_module('basicsr.archs.span_arch', BASICSR_DIR / 'archs' / 'span_arch.py')
    return span_mod.SPAN
...
def save_image_tensor(tensor: torch.Tensor, path: Path) -> None:
    # SPAN may leave values outside [0, 1]; clip before uint8 (known artifact fix).
...
def infer_folder(
    ...
    with torch.inference_mode():
        for img_path in tqdm(images, desc='SPAN'):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='SPAN folder inference adapter')
